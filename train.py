...
def main():
    ...
    if not os.path.exists('data/en_vocab.json') or not os.path.exists('data/zh_vocab.json'):
        print("Vocab files not found. Please run data/dataprocess.py first or we can run it now.")
        # For simplicity, let's assume they exist or user runs dataprocess.py
        # But to be safe, let's try to load data and build vocab if missing
    ...
    if args.model_type == 'rnn':
        ...
        if args.lr == 0.001:
            print("Using lower learning rate (0.0005) for Transformer stability.")
            args.lr = 0.0005
